...
def new_instance_from_config(config):
	...
	for distrib in env_capacity_distribs:
	    e.add_capacity_from_string(env,distrib,mas)
	# Agent population
	pop = p.new_instance(mas, u.cfg_pop_size(config), config)
	set_pop(mas, pop)
	# Cell rules
...
def run_one_cycle(mas,cfg):
	"""
		Run one experiment cycle of the MAS.
	"""
	matrix = e.get_cell_matrix(get_env(mas))
	pop = get_pop(mas)
	p.apply_population_rules(mas,pop,cfg,matrix)
	apply_cell_rules(mas)

def run_experiment(mas,cfg):
	"""
