...
CELL_MATRIX_IDX = 0      # The matrix of cells
MAX_CAPACITY_IDX = 1     # Maximum capacity any cell can bear 

# --- Defaults ---

DEFAULT_MAX_CAPACITY = 10.0    # Maximum capacity
...
def __empty_cell_matrix(env,sz,mas):
    ...
    for y in range(sz):
        row = []
        for x in range(sz):
            row.append(c.new_instance(env,mas))
        mat.append(row)
    return mat
...
def new_instance(mas, sz):
    ...
    # depend on it.
    set_max_capacity(env, DEFAULT_MAX_CAPACITY)
    mat = __empty_cell_matrix(env,sz,mas)
    #set_mas(env, mas)
...
def add_capacity_gaussian(env, max_capacity_factor, center, disp, mas):
    """
    ...
    min_capacity = max_capacity*1E-10
    # Compute the cell distance from the center at which to add
    # capacity.
    max_dist = math.ceil(disp*math.sqrt(-2*math.log(min_capacity)))   # FixMe: Error in formula!
    for x in range(cx-max_dist, cx+max_dist):
        for y in range(cx-max_dist, cx+max_dist):
            ...
            c.add_capacity(cell, capacity, mas)

def add_capacity_from_string(env, capacity_str,mas):
    eval(capacity_str)
...
def plot(env, showGrid=False):
    ...
    col = []
    sugar = []
    size_factor = 3000.0/sz/get_max_capacity(env)
